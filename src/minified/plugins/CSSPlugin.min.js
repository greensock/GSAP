 ...
 */
(window._gsQueue||(window._gsQueue=[])).push(function(){_gsDefine("plugins.CSSPlugin",["plugins.TweenPlugin","TweenLite"],function(V){var B=function(){V.call(this,"css");this._overwriteProps.length=0},ba,ca,r,da,L={},m=B.prototype=new V("css");m.constructor=B;B.version=1.665;B.API=2;B.defaultTransformPerspective=0;m="px";B.suffixMap={top:m,right:m,bottom:m,left:m,width:m,height:m,fontSize:m,padding:m,margin:m,perspective:m};var ea=/(?:\d|\-\d|\.\d|\-\.\d)+/g,pa=/(?:\d|\-\d|\.\d|\-\.\d|\+=\d|\-=\d|\+=.\d|\-=\.\d)+/g,
fa=/(?:\+=|\-=|\-|\b)[\d\-\.]+[a-zA-Z0-9]*(?:%|\b)/gi,W=/[^\d\-\.]/g,Ha=/(?:\d|\-|\+|=|#|\.)*/g,ga=/opacity *= *([^)]*)/,Ia=/opacity:([^;]*)/,Ja=/alpha\(opacity *=.+?\)/i,qa=/([A-Z])/g,ra=/-([a-z])/gi,Ka=/(^(?:url\(\"|url\())|(?:(\"\))$|\)$)/gi,sa=function(a,b){return b.toUpperCase()},La=/(?:Left|Right|Width)/i,Ma=/(M11|M12|M21|M22)=[\d\-\.e]+/gi,Na=/progid\:DXImageTransform\.Microsoft\.Matrix\(.+?\)/i,E=Math.PI/180,R=180/Math.PI,X={},M=document,S=M.createElement("div"),ha=M.createElement("img"),
N=B._internals={_specialProps:L},A=navigator.userAgent,ia,ta,ua,va,wa,ja,O,xa=A.indexOf("Android"),ya=M.createElement("div");wa=(ua=-1!==A.indexOf("Safari")&&-1===A.indexOf("Chrome")&&(-1===xa||3<Number(A.substr(xa+8,1))))&&6>Number(A.substr(A.indexOf("Version/")+8,1));va=-1!==A.indexOf("Firefox");/MSIE ([0-9]{1,}[\.0-9]{0,})/.exec(A);ja=parseFloat(RegExp.$1);ya.innerHTML="<a style='top:1px;opacity:.55;'>a</a>";O=(A=ya.getElementsByTagName("a")[0])?/^0.55/.test(A.style.opacity):!1;var za=function(a){return ga.test("string"===
...
this._linkCSSP(c._next,c._next._next,a._prev),this._target._gsOverwrittenClassNamePT=this._linkCSSP(c,this._target._gsOverwrittenClassNamePT),this._classNamePT=null,d=!0;return V.prototype._kill.call(this,b)||d};V.activate([B]);return B},!0)});window._gsDefine&&_gsQueue.pop()();